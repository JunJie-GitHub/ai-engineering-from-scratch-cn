...
> 导数 (Derivatives) 会告诉你下坡的方向。这正是神经网络 (Neural Network) 学习所需的一切。

**Type:** 学习
**Language:** Python
**Prerequisites:** 第一阶段，第 01-03 课
**Time:** 约 60 分钟

## 学习目标

- 计算常见机器学习 (machine learning) 函数（如 x^2、Sigmoid 函数 (Sigmoid)、交叉熵 (cross-entropy)）的数值导数 (numerical derivatives) 与解析导数 (analytical derivatives)
- 从零实现梯度下降 (gradient descent) 算法，以在一维和二维空间中最小化损失函数 (loss function)
- 推导线性回归模型 (linear regression model) 的梯度，并通过手动权重更新 (manual weight updates) 对其进行训练
- 解释海森矩阵 (Hessian matrix)、泰勒级数近似 (Taylor series approximations) 及其与优化方法 (optimization methods) 之间的联系

## 问题

你有一个包含数百万个权重 (Weights) 的神经网络 (Neural Network)。每个权重都相当于一个旋钮。你需要弄清楚该朝哪个方向转动每一个旋钮，才能让模型 (Model) 的误差稍微减小一些。微积分 (Calculus) 为你指明了这一方向。

如果没有微积分，训练 (Training) 神经网络就只能依靠随机调整，并期待最佳结果。而借助导数 (Derivatives)，你可以精确掌握每个权重如何影响误差 (Error)。从而确保每次都能将每个旋钮转向正确的方向。
...
### 步骤 1：从零开始计算数值导数 (Numerical Derivative)
...
数值导数与解析导数 (Analytical Derivative) 在多位小数上完全吻合。

### 步骤 2：偏导数 (Partial Derivatives) 与梯度 (Gradients)
...
### 步骤 3：使用梯度下降法 (Gradient Descent) 寻找 f(x) = x^2 的最小值
...
鞍点函数 (Saddle Function) 的海森矩阵特征值 (Eigenvalues) 为 2 和 -2（符号相反，证实为鞍点 (Saddle Point)）。碗状函数的特征值为 2 和 2（均为正，证实为最小值点）。

### 步骤 7：泰勒近似 (Taylor Approximation) 的实际应用
...
在 x0=0 附近，sin(x) ≈ x（一阶泰勒展开）。当 h 较小时近似效果极佳，但 h 较大时则会失效。这也是为什么梯度下降法在使用较小学习率 (Learning Rate) 时效果最佳的原因——每一步都假设线性近似是准确的。

### 步骤 8：这对神经网络 (Neural Network) 为何重要
...
所有基于梯度的训练循环 (Training Loop) 都遵循这一模式：预测、计算损失 (Loss)、计算梯度、更新权重 (Weights)。

## 使用方法

使用 NumPy，相同的操作会更快、更简洁：
...
你刚刚从零实现了梯度下降 (gradient descent)。PyTorch 会自动完成梯度计算 (gradient computation)，但更新循环 (update loop) 是完全相同的。

## 练习

1. 通过调用两次 `numerical_derivative` 来实现 `numerical_second_derivative(f, x)`。验证 x^3 在 x=2 处的二阶导数（second derivative）是否为 12。
2. 使用梯度下降（gradient descent）求解 f(x, y) = (x - 3)^2 + (y + 1)^2 的最小值。从 (0, 0) 开始。结果应收敛（converge）至 (3, -1)。
3. 在梯度下降循环中添加动量（momentum）：维护一个用于累积历史梯度（gradients）的速度向量（velocity vector）。在 f(x) = x^4 - 3x^2 上，对比使用与不使用动量时的收敛速度（convergence speed）。

## 关键术语

| 术语 | 通俗说法 | 实际含义 |
|------|----------------|----------------------|
| 导数 (Derivative) | “斜率” | 函数在某一点的变化率。表示输入每变化一个单位时，输出会变化多少。 |
| 偏导数 (Partial derivative) | “单变量导数” | 在保持其他所有变量不变的情况下，针对某一个变量求导。 |
| 梯度 (Gradient) | “最陡上升方向” | 由所有偏导数组成的向量。指向使函数值增长最快的方向。 |
| 梯度下降 (Gradient descent) | “下山” | 从参数中减去梯度（乘以学习率）以降低损失。神经网络训练的核心。 |
| 学习率 (Learning rate) | “步长” | 控制每次梯度下降步长大小的标量。过大：导致发散。过小：收敛缓慢。 |
| 链式法则 (Chain rule) | “导数相乘” | 复合函数求导的法则：df/dx = df/dg * dg/dx。反向传播的数学基础。 |
| 雅可比矩阵 (Jacobian) | “导数矩阵” | 当函数将向量映射为向量时，雅可比矩阵是由所有输出对输入的偏导数组成的矩阵。 |
| 数值导数 (Numerical derivative) | “有限差分” | 通过计算函数在两个邻近点处的值并求其斜率来近似导数。 |
| 反向传播 (Backpropagation) | “反向模式自动微分” | 利用链式法则，从输出层到输入层逐层计算梯度。神经网络的学习机制。 |
| 海森矩阵 (Hessian) | “二阶导数矩阵” | 由所有二阶偏导数组成的矩阵。描述函数的曲率。在临界点处，若海森矩阵正定，则表明该点为局部极小值。 |
| 泰勒级数 (Taylor series) | “多项式近似” | 利用函数在某一点的导数来近似该函数：f(x+h) ~ f(x) + f'(x)h + (1/2)f''(x)h^2 + ... 理解梯度下降和牛顿法为何有效的理论基础。 |
| 积分 (Integral) | “曲线下面积” | 某个量在特定范围内的累积。在机器学习中，积分用于定义概率、期望值和 KL 散度。 |

## 延伸阅读

- [3Blue1Brown：微积分的本质](https://www.3blue1brown.com/topics/calculus) - 导数 (derivative)、积分 (integral) 与链式法则 (chain rule) 的直观理解
- [斯坦福 CS231n：反向传播 (backpropagation)](https://cs231n.github.io/optimization-2/) - 梯度 (gradient) 如何在神经网络层 (neural network layer) 中传递